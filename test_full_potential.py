...
# Ensure backend directory is in python import path
script_dir = os.path.dirname(os.path.abspath(__file__))
backend_dir = os.path.join(script_dir, "backend") if not os.path.exists(os.path.join(script_dir, "app")) else script_dir
...
async def run_full_potential_audit():
    ...
    # 3. Test Enterprise Vector Memory Engine & Dataset Seeding
    print("\n3️⃣  Testing ChromaDB Vector Memory Engine & Dataset Auto-Seeding...")
    from app.memory.manager import memory_manager
    mem_init_ok = await memory_manager.initialize()
    ...
    print(f"   ✓ Memory Engine Initialized: {'PASSED' if mem_init_ok else 'FAILED'}")
    ...
    # 5. Test 10-Agent Swarm Orchestrator Mesh & Concrete Execution
    ...
    from app.multi_agent.agent_pool import coding_agent, memory_agent
    ...
    # Test concrete specialized subtask execution
    coding_res = await coding_agent.execute_task(SubTaskSpec(assigned_agent=AgentRole.CODING, goal="Analyze system architecture"))
    memory_res = await memory_agent.execute_task(SubTaskSpec(assigned_agent=AgentRole.MEMORY, goal="Query swarm architecture"))
    ...
    print(f"   ✓ Coding Agent Execution Status: {coding_res.status.value}")
    print(f"   ✓ Memory Agent Execution Status: {memory_res.status.value}")
